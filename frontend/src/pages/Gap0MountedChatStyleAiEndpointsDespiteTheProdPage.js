...
export default function Gap0MountedChatStyleAiEndpointsDespiteTheProdPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-0-mounted-chat-style-ai-endpoints-despite-the-prod/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>0 mounted /chat-style AI endpoints despite the product premise</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-0-mounted-chat-style-ai-endpoints-despite-the-prod/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
