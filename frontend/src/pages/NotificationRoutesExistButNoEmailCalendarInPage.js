...
export default function NotificationRoutesExistButNoEmailCalendarInPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-notification-routes-exist-but-no-email-calendar-in/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Notification routes exist but no email/calendar invite delivery</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-notification-routes-exist-but-no-email-calendar-in/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
