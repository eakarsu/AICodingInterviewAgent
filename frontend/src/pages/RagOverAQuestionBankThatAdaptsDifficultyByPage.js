...
// Feature: RAG over a question bank that adapts difficulty by role and seniority
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverAQuestionBankThatAdaptsDifficultyByPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-a-question-bank-that-adapts-difficulty-by/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over a question bank that adapts difficulty by role and seniority</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-a-question-bank-that-adapts-difficulty-by/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
