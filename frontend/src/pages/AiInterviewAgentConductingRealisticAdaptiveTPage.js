...
// Feature: AI interview agent conducting realistic, adaptive technical interviews with rubric scoring
import React, { useState } from 'react';
import api from '../services/api';

export default function AiInterviewAgentConductingRealisticAdaptiveTPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-ai-interview-agent-conducting-realistic-adaptive-t/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>AI interview agent conducting realistic, adaptive technical interviews with rubric scoring</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-ai-interview-agent-conducting-realistic-adaptive-t/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
