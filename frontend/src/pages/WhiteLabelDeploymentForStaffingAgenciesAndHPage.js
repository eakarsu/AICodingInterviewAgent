...
// Feature: White-label deployment for staffing agencies and hiring consultancies
import React, { useState } from 'react';
import api from '../services/api';

export default function WhiteLabelDeploymentForStaffingAgenciesAndHPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-white-label-deployment-for-staffing-agencies-and-h/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>White-label deployment for staffing agencies and hiring consultancies</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-white-label-deployment-for-staffing-agencies-and-h/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
